...
class Usps extends \Opencart\System\Engine\Controller {
	public function index(): void {
		...
		$data['breadcrumbs'][] = [
			...
			'href' => $this->url->link('extension/open929/shipping/usps', 'user_token=' . $this->session->data['user_token'])
		];

		$data['save'] = $this->url->link('extension/open929/shipping/usps|save', 'user_token=' . $this->session->data['user_token']);
		...
		$this->response->setOutput($this->load->view('extension/open929/shipping/usps', $data));
	}
	...
	public function save(): void {
		...
		if (!$this->user->hasPermission('modify', 'extension/open929/shipping/usps')) {
		...
		if (!$this->request->post['shipping_usps_user_id']) {
			$json['user_id'] = $this->language->get('error_user_id');
		}
		...
		if (!$this->request->post['shipping_usps_width']) {
			$json['dimension'] = $this->language->get('error_width');
		}

		if (!$this->request->post['shipping_usps_height']) {
			$json['dimension'] = $this->language->get('error_height');
		}
